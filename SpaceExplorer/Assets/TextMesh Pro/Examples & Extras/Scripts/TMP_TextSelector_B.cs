...
namespace TMPro.Examples
{
    ...
    public class TMP_TextSelector_B : MonoBehaviour, IPointerEnterHandler, IPointerExitHandler, IPointerClickHandler, IPointerUpHandler
    {
        ...
        void RestoreCachedVertexAttributes(int index)
        {
            ...
            // Restore UV0S
			// UVS0
			CopyUVs(m_cachedMeshInfoVertexData[materialIndex].uvs0, m_TextMeshPro.textInfo.meshInfo[materialIndex].uvs0, vertexIndex);

			// UVS2
			CopyUVs(m_cachedMeshInfoVertexData[materialIndex].uvs2, m_TextMeshPro.textInfo.meshInfo[materialIndex].uvs2, vertexIndex);
			...
			// UVS0
			CopyUVs(m_cachedMeshInfoVertexData[materialIndex].uvs0, m_TextMeshPro.textInfo.meshInfo[materialIndex].uvs0, lastIndex);

			// UVS2
			CopyUVs(m_cachedMeshInfoVertexData[materialIndex].uvs2, m_TextMeshPro.textInfo.meshInfo[materialIndex].uvs2, lastIndex);

            // Need to update the appropriate 
            m_TextMeshPro.UpdateVertexData(TMP_VertexDataUpdateFlags.All);
        }

		// Overloads to handle different TMP versions where UV arrays may be Vector2[] or Vector4[]
		private static void CopyUVs(Vector2[] src, Vector2[] dst, int vertexIndex)
		{
			dst[vertexIndex + 0] = src[vertexIndex + 0];
			dst[vertexIndex + 1] = src[vertexIndex + 1];
			dst[vertexIndex + 2] = src[vertexIndex + 2];
			dst[vertexIndex + 3] = src[vertexIndex + 3];
		}

		private static void CopyUVs(Vector4[] src, Vector4[] dst, int vertexIndex)
		{
			dst[vertexIndex + 0] = src[vertexIndex + 0];
			dst[vertexIndex + 1] = src[vertexIndex + 1];
			dst[vertexIndex + 2] = src[vertexIndex + 2];
